...
<div class="login-container"> <!-- 📦 Contenedor principal del login -->
    <div class="logo-container"> <!-- 🖼️ Contenedor del logo -->
        <img src="<?= BASE_URL ?>/recursos/img/logo.jpg" alt="Logo de la aplicación" class="logo-login">
    </div>

    <h2>Iniciar Sesión</h2> <!-- 🧾 Título del formulario -->

    <?php if (isset($error)) : ?> <!-- ⚠️ Muestra error si existe -->
        <div class="error-msg"><?= htmlspecialchars($error) ?></div> <!-- 🚨 Mensaje de error escapado -->
    <?php endif; ?>

   <form action="<?= BASE_URL ?>/publico/index.php?accion=login" method="POST">
    ...
    <p class="link-registro"> <!-- 🔗 Enlace a registro -->
        ¿No tienes cuenta?
        <a href="<?= BASE_URL ?>/publico/index.php?accion=registro">Regístrate aquí</a> <!-- 📝 Link a registro -->
    </p>
</div>
